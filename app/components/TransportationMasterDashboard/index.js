...
class TransportationMasterDashboard extends React.Component {
    ...
    componentDidMount(){
        ...
        fetch("https://2bb6d5jv76.execute-api.ap-south-1.amazonaws.com/DEV/transportationbilling", configure)
        .then((res)=>res.json())
        .then((data)=>{
            console.log(data)
                this.setState({
    ...
    salecontractChange = (e)=>{
        ...
        fetch("https://2bb6d5jv76.execute-api.ap-south-1.amazonaws.com/DEV/transportationbilling", salecontractoption)
        .then((res)=>res.json())
        .then((data)=>{
    ...
    handleSave = () => {
        ...
        fetch("https://2bb6d5jv76.execute-api.ap-south-1.amazonaws.com/DEV/transportationbilling", saveoption)
        .then((res)=>res.json())
        .then((data)=>{
            console.log(data)
            this.handleViewDetails();
                                            ...
                                            <div className="tabel_scroll">
                                                            ...
                                                            <table style={{ border: "1px solid #c1e3ff" }}>
                                                                ...
                                                                <tbody>
                                                                        ...
                                                                        <tr>
                                                                            <td style={{ width: "12%" }}>
                                                                            <Input placeholder="Sale contract" 
                                                                            onChange={this.salecontractChange}
                                                                            value={this.state.saleContract}
                                                                            />
                                                                            </td>
                                                                            <td style={{ width: "12%" }}>
                                                                            <Input placeholder="Customer Code" value={this.state.customerCode} disabled />
                                                                            </td>
                                                                            <td style={{ width: "12%" }}>
                                                                            <Input placeholder="cost center" value={this.state.costCenter} disabled />

                                                                            </td>
                                                                            <td style={{ width: "12%" }}>
                                                                            <Select  placeholder="Select" style={{ width: 160 }} onChange={this.handlerefdoctype}>
                                                                            {this.state.refDocType.map((i, index)=>{
                                                                                return(
                                                                                    <>
                                                                                    <Option title="ref_doc" value={i} key={index}>
                                                                                        {i}
                                                                                    </Option>
                                                                            ...
                                                                            <td style={{ width: "12%" }}>
                                                                            <Select placeholder="Select" style={{ width: 160 }} onChange={this.handlebillingbasis}>
                                                                               
                                                                            {this.state.billingBasis.map((i, index)=>{
                                                                                {console.log(i,index)}
                                                                                return(
                                                                                    <>
                                                                                    <Option title="billing_basis" value={i} key={index}>
                                                                                        {i}
                                                                                    </Option>
                                                                            ...
                                                                            <td style={{ width: "12%" }}>
                                                                            <Select placeholder="Select" style={{ width: 170 }} onChange={this.handleinvoicetype} >
                                                                                {
                                                                                    this.state.invoiceType.map((i,index)=>{
                                                                                        return(
                                                                                            <>
                                                                                            <Option title="invoice_type" value={i} key={index}>{i}</Option>
                                                                                            </>
                                                                                        )
                                                                                    })
                                                                                }
                                                                            ...
                                                                            <td style={{ width: "12%" }}>
                                                                            <Input placeholder="Account usage" onChange={this.handleaccountusage}/>
                                                                            
                                                                            </td>
                                                                            <td style={{ width: "5%" }}>
                                                                            <Button type="primary" icon={<PlusOutlined />} onClick={this.handleSave} >
                                                                                 Save
                                                                            </Button>
                                                                            {this.state.isSave && <p style={{marginTop:"5px",color:"blue"}}>Saved successfully</p>}
                                                                            </td>
                                                    ...
                                                    <table style={{ border: "1px solid #c1e3ff" }}>
                                                        ...
                                                        <tbody>
                                                            {this.state.viewDetailList.map((i, index)=>{
                                                                ...
                                                                <td style={{ width: "12%" }}>{i.salecontractno}</td>
                                                                <td style={{ width: "12%" }}>{i.customercode}</td>
                                                                <td style={{ width: "12%" }}>{i.costcenter}</td>
                                                                <td style={{ width: "12%" }}>{i.refdoctype}</td>
                                                                <td style={{ width: "12%" }}>{i.billingbasis}</td>
                                                                <td style={{ width: "12%" }}>{i.invoicetype}</td>
                                                                <td style={{ width: "12%" }}>{i.accountusage}</td>
                                                                </tr>
                                                                )
                                                            })}
